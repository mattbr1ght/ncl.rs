...
fn prompt_template_selection() -> Result<Template> {
    let templates = load_templates().context("Failed to load templates")?;

    if templates.is_empty() {
        return Err(anyhow::anyhow!(
            "No templates available. Please ensure templates are installed."
        ));
    }
...
fn display_missing_dependencies(template: &Template) -> Result<()> {
    ...
    if !missing.is_empty() {
        ...
        return Err(anyhow!(
            "Missing dependencies error: Can't run the project without dependencies"
        ));
    }
    Ok(())
}
...
pub fn run(skip_github: bool, skip_coolify: bool, skip_trello: bool) -> Result<()> {
    ...
    let mut config = NclConfig::load().context("Failed to load NCL configuration")?;
