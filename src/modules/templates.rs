use crate::modules::common::{Dependency, Installable, ncl_config_dir};
...
impl Template {
    ...
    pub fn get_post_install_hook(&self) -> &Hook {
        ...
        self.hooks
            .get("post_install")
            .or_else(|| {
                if !self.post_install_hook.is_empty() {
...
fn ensure_universal_base_exists() -> Result<()> {
    ...
    if source_path.exists() && source_path.join("template.toml").exists() {
        std::fs::create_dir_all(&universal_path).with_context(|| {
            format!(
                "Failed to create universal-base directory: {}",
...
pub fn write_default_templates(target_path: &Path) -> Result<()> {
    ...
    DEFAULT_TEMPLATES.extract(target_path).with_context(|| {
        format!(
            "Failed to extract default templates to {}",
            target_path.display()
        )
    })?;
    ...
    if universal_source.exists() {
        ...
        std::fs::create_dir_all(universal_dest.parent().unwrap()).with_context(|| {
            format!(
                "Failed to create universal-base parent directory: {}",
                universal_dest.parent().unwrap().display()
            )
        })?;
        ...
        debug!(
            "Moved universal-base to separate directory: {}",
