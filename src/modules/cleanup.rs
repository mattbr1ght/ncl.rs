...
use crate::modules::config::{ProjectConfig, NclConfig};
...
pub fn remove_project(project_path: &PathBuf) -> Result<()> {
    ...
    let global_config = NclConfig::load()
        .context("Failed to load global configuration")?;
    
    // Delete GitHub repository
    if let (Some(owner), Some(repo)) = (&project_config.github_repo_owner, &project_config.github_repo_name) {
        if let Ok(Some(token)) = global_config.get_github_token() {
            ...
            match github::delete_repository(&token, owner, repo) {
                Ok(()) => cliclack::log::success("GitHub repository deleted")?,
                Err(e) => cliclack::log::warning(&format!("Failed to delete GitHub repository: {}", e))?,
            }
        }
    }
        ...
        if let Some(endpoint) = project_config.coolify.api_endpoint.as_deref() {
            if let Ok(Some(token)) = global_config.get_coolify_token() {
                ...
                match client.delete_project(project_id) {
                    Ok(()) => cliclack::log::success("Coolify project deleted")?,
                    Err(e) => cliclack::log::warning(&format!("Failed to delete Coolify project: {}", e))?,
                }
            }
        }
    }
